                    ...
                    if link.type == "internal":
                        ...
                        if links_to != "":
                            ...
                            if mention_position != -1:
                                ...
                                links_to_encoded_title = encode_id(
                                        ...
                                        links.append(
                                            {
                                                ...
                                                "end": end,
                                                "text": link_text,
                                                "links_to": links_to_encoded_title,  # To mach documents when searching
                                                "source_doc": encode_id(
                                                    page_obj.title, encode_title=True
                                                ),
...
def index_links(self, links: List[dict]):
    """
    Index links in the database
    """
    # links_batch = [mention for batch in links for mention in batch
    operations = []
    # Bulk upsert
    # https://stackoverflow.com/questions/5292370/fast-or-bulk-upsert-in-pymongo
    for article_mention in links:
        operations.append(
            UpdateOne(
    ...
    print(f"Inserting {n_links}")
    if operations:
        self.links_collection.bulk_write(operations)
        print(f"Inserted {n_links}")
